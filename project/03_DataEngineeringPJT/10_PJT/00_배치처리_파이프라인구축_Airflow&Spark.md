...
- 이번 프로젝트의 목표는 Airflow를 활용한 스케줄링(Workflow Orchestration)과 Spark를 이용한 배치 처리를 구현하는 것입니다. 
  - ex. 매일 새벽 1시에 실행되는 배치 작업을 통해, 전일 콘텐츠 수집 및 사용자 별 혹은 사용자 전체의 로그 데이터를 집계/분석하고, **키워드 트렌드나 카테고리별 통계**를 산출하여 리포트를 자동 생성합니다.
- 배치가 완료된 후에는 원본 데이터(JSON 등)를 별도의 아카이빙 폴더로 이동하여, 전체 데이터 관리가 용이하도록 설계합니다. 
  - 이후 HDFS를 다루게 되면 HDFS 형태로 저장
- Airflow DAG을 구성하여 배치 스케줄링(ex. 매일 새벽 1시 실행)할 수 있다.
- Spark를 활용하여 전처리/분석 작업을 수행할 수 있다.
- 전날 데이터를 집계 후, 트렌드/키워드 등의 리포트를 PDF를 자동 생성할 수 있다.
- 원본 데이터를 아카이빙 폴더로 이동하는 작업을 수행할 수 있다.
- 배치 작업 정리
  - 리포트 생성
  - 원본 데이터 아카이빙
    - 필요한 작업 수행 후 저장하는 방식
    - 저장소에 저장 후 추가 분석, 가공하는 방식
- 현재 프로젝트 흐름
  - Web 크롤링을 통한 데이터 수집
  - Kafka를 통한 실시간 데이터 스트리밍 (Extract)
  - Flink로 데이터 전처리 후 (Transformation) PostgreSQL에 적재 (Load)
    - 카테고리 분류
    - 키워드 추출
    - 임베딩
  - 이러한 ETL 구조에서 flink 이후 Spark를 활용한 배치 처리 및 Airflow 스케줄링을 통해 리포트 생성 및 데이터 아카이빙 작업 수행
    - 분석 리포트 생성
    - 장기 저장소로 데이터 이동(HDFS 등)
  - 배치처리 분석을 위해서는 파일 형태로 저장된 데이터가 필요
    - JSON 형태로 HDFS에 저장된 데이터를 Spark로 읽어와 분석 수행
    - JSON 형태로 저장하는 것은 Flink에서 처리
...
- 개발언어/프로그램
  - Python
    - 스크립트(배치 로직, Spark 처리 등)
  - Apache Spark
    - 데이터프레임, RDD 기반 분석 및 시각화(로컬 파일 시스템에서 데이터 로드)
  - Apache Airflow
    - 배치 스케줄링, Workflow Orchestration
  - Matplotlib
    - 리포트용 시각화(차트, 그래프)
  - 파일 및 디렉터리 관리
    - Python 표준 라이브러리(os, shutil) 활용

## 구현 방법
### 1) Airflow DAG 구성
  - 주기적으로 특정 시점에 실행되는 DAG를 구성
    - ex. 매일 새벽 1시
...
- 분석에 사용된 JSON 파일들을 작업이 끝나면 ../long_term_archive 디렉터리로 이동  
  - 이후 HDFS로 장기저장소로 변경할 것
    - 어떤 데이터가 분석 대상이고, 이미 분석이 끝난 데이터인지 구분하기 위해서라도 단기/장기 저장소 구분 필요
    - 분석한 데이터는 장기로 이동
    - 단기 저장소에는 분석되지 않은 데이터만 남도록 관리
...
### PJT 실습
`10_PJT\batch\dags\daily_report_dag.py` 참고
...
- Airflow + Spark를 이용한 배치 파이프라인 구축
  - 매일 특정 시간(ex. 자정, 새벽 1시 등)에 자동 실행
...
### 원본 데이터 아카이빙 + 2차 가공
- 배치 처리 완료 후, 원본 파일을 ../long_term_archive 디렉터리로 이동 (장기저장용)
